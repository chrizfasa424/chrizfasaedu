        ...
        <form action="{{ route('messages.store') }}" method="POST" class="divide-y divide-slate-100">
            ...
            <div class="px-6 py-5 space-y-5">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4" id="audienceGrid">
                        ...
                        <label class="audience-card cursor-pointer" data-audience="{{ $val }}">
                            <input type="radio" name="audience" value="{{ $val }}" class="sr-only"
                                   {{ old('audience', 'all_students') === $val ? 'checked' : '' }}>
                            <div class="flex flex-col items-center gap-2 rounded-xl border-2 px-3 py-4 text-center transition
                                        {{ old('audience', 'all_students') === $val
                                            ? 'border-[#2D1D5C] bg-[#2D1D5C] text-white shadow-md'
                                            : 'border-slate-200 bg-white text-slate-600 hover:border-[#2D1D5C]/40' }}
                                        audience-option">
                ...
                <div id="classRow" style="display:{{ old('audience') === 'class' ? 'block' : 'none' }}">
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5" for="class_id">Select Class</label>
                    <select id="class_id" name="class_id"
                            class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 shadow-sm focus:border-[#2D1D5C] focus:outline-none focus:ring-1 focus:ring-[#2D1D5C]">
                ...
                {{-- Subject --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5" for="subject">Subject</label>
                    <input type="text" id="subject" name="subject" value="{{ old('subject') }}"
                           placeholder="Enter message subject…"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-800 shadow-sm focus:border-[#2D1D5C] focus:outline-none focus:ring-1 focus:ring-[#2D1D5C]
                ...
                {{-- Body --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5" for="body">Message</label>
                    <textarea id="body" name="body" rows="10"
                              placeholder="Type your message here…"
                              class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-800 shadow-sm focus:border-[#2D1D5C] focus:outline-none focus:ring-1 focus:ring-[#2D1D5C] resize-none
            ...
            <div class="flex items-center justify-between gap-4 bg-slate-50/70 px-6 py-4">
                <a href="{{ route('messages.index') }}" class="text-sm font-medium text-slate-500 hover:text-slate-700">
                    ← Back
                </a>
                <button type="submit"
                        class="inline-flex items-center gap-2 rounded-xl bg-[#2D1D5C] px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#3A2872] active:scale-95">
...
document.querySelectorAll('.audience-card').forEach(card => {
    card.addEventListener('click', () => {
        ...
        const sel = card.querySelector('.audience-option');
        sel.classList.add('border-[#2D1D5C]', 'bg-[#2D1D5C]', 'text-white', 'shadow-md');
        sel.classList.remove('border-slate-200', 'bg-white', 'text-slate-600');

        document.getElementById('classRow').style.display = val === 'class' ? 'block' : 'none';
    });
});
