...
<div class="space-y-8">
    ...
    >
        ...
        <div class="relative z-10 flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8 lg:p-10">
            <div class="flex items-center gap-5">
                <div
                    class="flex h-20 w-20 items-center justify-center rounded-2xl border-2 border-white/15 text-3xl font-extrabold shadow-xl"
                ...
                <div>
                    ...
                    <h1 class="mt-1 text-2xl font-extrabold leading-tight text-white sm:text-3xl">
                        Welcome, {{ $parentAccountName }}
                    </h1>
                    <p class="mt-2 text-sm text-white/60">
                        Overview of your {{ $totalChildren === 1 ? 'child' : $totalChildren . ' children' }}.
                    </p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-3 sm:flex-col sm:items-end">
                ...
                @if($totalOutstanding > 0)
                    <div class="rounded-2xl border border-red-300/30 bg-red-500/20 px-5 py-3 text-right backdrop-blur">
                        <p class="text-xs font-semibold uppercase tracking-wider text-white/50">Outstanding</p>
                        <p class="mt-0.5 text-xl font-extrabold text-red-300">NGN {{ number_format($totalOutstanding, 2) }}</p>
                    </div>
                @else
                    <div class="rounded-2xl border border-emerald-300/30 bg-emerald-500/20 px-5 py-3 text-right backdrop-blur">
    ...
    <section id="children-overview" class="scroll-mt-32 space-y-6">
        <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 class="text-lg font-bold text-slate-900">Academic Overview</h2>
            <p class="mt-1 text-sm text-slate-500">Quick summary across all linked children.</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-3">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p class="mt-1 text-xs font-semibold text-slate-500">Recent Results Loaded</p>
            </div>
            ...
            <div class="col-span-2 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-1">
                ...
                @if($totalOutstanding > 0)
                    <p class="mt-4 text-2xl font-extrabold text-red-600">NGN {{ number_format($totalOutstanding, 2) }}</p>
            ...
            <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4" style="background: {{ $primaryColor }}0a">
                    <div class="flex items-center gap-4">
                        @if($child->photo)
                            <img src="{{ asset('storage/' . ltrim($child->photo, '/')) }}" alt="{{ $child->full_name }}" class="h-12 w-12 rounded-2xl border border-slate-200 object-cover">
                        @else
                            <div class="flex h-12 w-12 items-center justify-center rounded-2xl text-base font-extrabold" style="background:{{ $primaryColor }}1a;color:{{ $primaryColor }}">
                                {{ \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($child->first_name, 0, 1)) }}
                            </div>
                        @endif
                        <div>
                            <h3 class="text-base font-bold text-slate-900">{{ $child->full_name }}</h3>
                    ...
                    @if($data['outstanding_fees'] > 0)
                        <div class="rounded-2xl border border-red-200 bg-red-50 px-4 py-2 text-right">
                            ...
                            <p class="text-sm font-extrabold text-red-700">NGN {{ number_format($data['outstanding_fees'], 2) }}</p>
                        </div>
                    @else
                        <span class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700">Fees Cleared</span>
                    @endif
                </div>
    ...
    <section id="results-grades" class="scroll-mt-32 space-y-4">
        <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 class="text-lg font-bold text-slate-900">Results and Grades</h2>
        ...
        @forelse($childrenData as $data)
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-100 px-6 py-4">
                    <p class="text-base font-bold text-slate-900">{{ $child->full_name }}</p>
                    <p class="text-xs text-slate-500">
        ...
        @empty
            <div class="rounded-3xl border border-dashed border-slate-300 bg-white py-10 text-center">
                <p class="text-sm font-semibold text-slate-400">No result records to display yet.</p>
            </div>
        @endforelse
    </section>

    <section id="fees-summary" class="scroll-mt-32 space-y-4">
        <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 class="text-lg font-bold text-slate-900">Fees Summary</h2>
            <p class="mt-1 text-sm text-slate-500">Outstanding fee balance per child.</p>
        </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                @foreach($childrenData as $data)
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                        ...
                        <div class="mt-4 flex items-center justify-between rounded-2xl border px-4 py-3 {{ $hasOutstanding ? 'border-red-200 bg-red-50' : 'border-emerald-200 bg-emerald-50' }}">
                            <span class="text-xs font-semibold uppercase tracking-wide {{ $hasOutstanding ? 'text-red-600' : 'text-emerald-700' }}">
                                {{ $hasOutstanding ? 'Outstanding' : 'Status' }}
                            </span>
                            <span class="text-sm font-extrabold {{ $hasOutstanding ? 'text-red-700' : 'text-emerald-700' }}">
        ...
        @else
            <div class="rounded-3xl border border-dashed border-slate-300 bg-white py-10 text-center">
                <p class="text-sm font-semibold text-slate-400">No children linked yet, so no fee summary is available.</p>
            </div>
        @endif
    </section>
</div>
