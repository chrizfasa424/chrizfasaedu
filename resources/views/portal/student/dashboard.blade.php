...
<div class="space-y-6">
    ...
    <section id="results-section" class="rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <div>
                <h2 class="text-base font-bold text-slate-900">Recent Approved Results</h2>
                <p class="text-xs text-slate-400 mt-0.5">Showing latest {{ $results->count() }} approved subject results</p>
            </div>
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="bg-slate-50">
                        ...
                        <th class="px-4 py-3 text-center text-[11px] font-semibold uppercase tracking-wider text-slate-500">CA</th>
                        ...
                        <th class="px-6 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500">Remark</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td class="px-4 py-3.5 text-center text-slate-600">
                            @php $caTotal = ($result->ca1_score ?? 0) + ($result->ca2_score ?? 0) + ($result->ca3_score ?? 0); @endphp
                            {{ $caTotal > 0 ? $caTotal : '—' }}
                        </td>
                        ...
                        <td class="px-6 py-3.5 text-slate-500">{{ \Illuminate\Support\Str::limit((string) ($result->teacher_remark ?? ''), 60) ?: '—' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
