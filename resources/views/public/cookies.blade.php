...
<head>
    ...
    @php
        ...
        $seoCookiesTitle = 'Cookies Policy | ' . $schoolName;
        $seoCookiesDescription = $cookiesIntro !== '' ? $cookiesIntro : 'Learn how this website uses cookies and how you can manage them.';
        $seoCookiesLogo = $school?->logo ? asset('storage/' . ltrim($school->logo, '/')) : '';
    @endphp
    @include('public.partials.seo', [
        'title' => $seoCookiesTitle,
        'description' => $seoCookiesDescription,
        'canonical' => route('public.cookies'),
        'type' => 'website',
        'schemaType' => 'WebPage',
        'siteName' => $schoolName,
        'image' => $seoCookiesLogo,
        'school' => $school,
    ])
    @if($faviconPath)
        ...
        <main class="bg-pattern-grid">
            <section class="legal-hero-shell bg-gradient-to-br from-slate-50 to-white" data-reveal>
                <div class="mx-auto max-w-5xl px-6 py-14 lg:px-8 lg:py-16" data-reveal="up">
                    ...
                    <h1 class="public-premium-title mt-3 font-display text-4xl font-semibold text-ink sm:text-5xl">{{ $cookiesTitle }}</h1>
                    @if($cookiesIntro !== '')
