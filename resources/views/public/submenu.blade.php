                                ...
                                <div id="submenu-{{ $section['id'] }}" data-menu-panel class="absolute {{ $alignClass }} top-full z-50 hidden w-[22rem] max-w-[calc(100vw-2rem)] pt-3">
                                    ...
                                    >
                                        ...
                                        @foreach($section['items'] as $menuItem)
                                            <a href="{{ route('public.submenu', ['section' => $section['id'], 'slug' => $menuItem['slug']]) }}" class="theme-submenu-link block rounded-lg px-3 py-2 text-sm font-medium transition duration-200 {{ $section['id'] === $sectionKey && $menuItem['slug'] === $item['slug'] ? 'theme-submenu-link-active' : '' }}">
                                                {{ $menuItem['title'] }}
                                            </a>
                                        @endforeach
                                    </div>
                                ...
                                @if(!empty($section['items']))
                                    ...
                                    <div id="mobile-submenu-{{ $section['id'] }}" data-mobile-submenu-panel class="hidden px-4 pb-4" style="--submenu-primary: {{ $submenuPrimaryColor }}; --submenu-secondary: {{ $submenuSecondaryColor }}; --submenu-hover-text: {{ $submenuHoverTextColor }};">
                                        <div class="space-y-1 border-l border-slate-200 pl-4">
                                            ...
                                            @foreach($section['items'] as $menuItem)
                                                <a href="{{ route('public.submenu', ['section' => $section['id'], 'slug' => $menuItem['slug']]) }}" class="theme-mobile-submenu-link block rounded-lg px-3 py-2 text-sm transition duration-200 {{ $section['id'] === $sectionKey && $menuItem['slug'] === $item['slug'] ? 'theme-mobile-submenu-link-active' : '' }}">{{ $menuItem['title'] }}</a>
                                            @endforeach
                                        </div>
                                    </div>
                                @else
