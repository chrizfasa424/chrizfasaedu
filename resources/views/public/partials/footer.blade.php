...
<footer class="text-slate-200" style="background-color: {{ $footerBgColor }}; color: {{ $footerMutedColor }};">
    <div class="mx-auto max-w-7xl px-6 py-14 lg:px-8">
        ...
        <div class="mt-10 h-px w-full" style="background-color: {{ $footerSecondaryLineColor }};"></div>

        <div class="pt-6 text-xs" style="color: {{ $footerMutedColor }};">
            <p>&copy; {{ date('Y') }} {{ $schoolName }}. {{ $footerNote }}</p>
            <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1">
                @foreach($legalLinks as $legalLink)
                    <a href="{{ $legalLink['url'] }}" class="transition hover:text-white">{{ $legalLink['label'] }}</a>
                @endforeach
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="max-w-3xl">
                ...
                <p class="mt-2 text-sm leading-relaxed text-slate-600">
                    {{ $cookieBannerText }}
                    See our <a href="{{ route('public.cookies') }}" class="font-semibold text-brand-700 underline underline-offset-2">Cookies Policy</a> for details.
                </p>
            </div>
