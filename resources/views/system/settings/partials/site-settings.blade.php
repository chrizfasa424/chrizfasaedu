<form action="{{ route('settings.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $school->name) }}" class="w-full rounded-2xl border-slate-300">
        </div>
        <div>
            <label class="mb-2 block text-sm font-semibold text-slate-700">Email Address</label>
            <input type="email" name="email" value="{{ old('email', $school->email) }}" class="w-full rounded-2xl border-slate-300">
        </div>
        <div>
            <label class="mb-2 block text-sm font-semibold text-slate-700">Phone Number</label>
            <input type="text" name="phone" value="{{ old('phone', $school->phone) }}" class="w-full rounded-2xl border-slate-300">
        </div>
        <div>
            <label class="mb-2 block text-sm font-semibold text-slate-700">Website</label>
            <input type="url" name="website" value="{{ old('website', $school->website) }}" class="w-full rounded-2xl border-slate-300" placeholder="https://example.com">
        </div>
        <div>
            <label class="mb-2 block text-sm font-semibold text-slate-700">Main School Domain</label>
            <input type="text" name="domain" value="{{ old('domain', $school->domain) }}" class="w-full rounded-2xl border-slate-300" placeholder="chrizfasaedu.test">
        ...
        <div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                @if($school->domain)
                    <a href="http://{{ $school->domain }}" target="_blank" rel="noopener" class="font-semibold text-[#2D1D5C] hover:underline">http://{{ $school->domain }}</a>
                @else
    ...
    <div>
        ...
        <input type="text" name="motto" value="{{ old('motto', $school->motto) }}" class="w-full rounded-2xl border-slate-300">
    </div>

    <div>
        <label class="mb-2 block text-sm font-semibold text-slate-700">Address</label>
        <textarea name="address" rows="4" class="w-full rounded-2xl border-slate-300">{{ old('address', $school->address) }}</textarea>
    </div>
    ...
    <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div>
                ...
                <input type="text" form="public-site-settings-form" name="header_apply_text" value="{{ old('header_apply_text', $publicPage['header_apply_text'] ?? 'Apply') }}" class="w-full rounded-2xl border-slate-300">
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Header Portal Text</label>
                <input type="text" form="public-site-settings-form" name="header_portal_login_text" value="{{ old('header_portal_login_text', $publicPage['header_portal_login_text'] ?? 'Portal Login') }}" class="w-full rounded-2xl border-slate-300">
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Mobile Apply Text</label>
                <input type="text" form="public-site-settings-form" name="mobile_apply_text" value="{{ old('mobile_apply_text', $publicPage['mobile_apply_text'] ?? 'Apply Now') }}" class="w-full rounded-2xl border-slate-300">
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Mobile Portal Text</label>
                <input type="text" form="public-site-settings-form" name="mobile_portal_login_text" value="{{ old('mobile_portal_login_text', $publicPage['mobile_portal_login_text'] ?? 'Portal Login') }}" class="w-full rounded-2xl border-slate-300">
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Site Title Suffix</label>
                <input type="text" form="public-site-settings-form" name="site_title_suffix" value="{{ old('site_title_suffix', $publicPage['site_title_suffix'] ?? 'KG, Primary and Secondary School') }}" class="w-full rounded-2xl border-slate-300">
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Mobile Menu Title</label>
                <input type="text" form="public-site-settings-form" name="mobile_menu_title" value="{{ old('mobile_menu_title', $publicPage['mobile_menu_title'] ?? 'Menu') }}" class="w-full rounded-2xl border-slate-300">
            </div>
        </div>
    </div>
