...
<head>
    ...
    <?php if($layoutFavicon): ?>
        <link rel="icon" type="image/png" href="<?php echo e(asset('storage/' . ltrim($layoutFavicon, '/'))); ?>">
    <?php endif; ?>
...
<body class="min-h-screen bg-[#F4F6FB] text-slate-900">
    ...
    <div class="min-h-screen">
                ...
                <div class="border-b border-white/10 bg-gradient-to-r from-[#24174A] via-[#2D1D5C] to-[#3A2872] px-6 pb-5 pt-6">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <?php if($schoolLogo): ?>
                                <img src="<?php echo e(asset('storage/' . ltrim($schoolLogo, '/'))); ?>" alt="<?php echo e($schoolBrandName); ?> logo" class="h-14 w-14 rounded-2xl border border-white/15 bg-white object-cover shadow-lg shadow-black/15">
                            <?php else: ?>
                            ...
                            <div>
                                ...
                                ?>
                                <p class="text-[11px] font-semibold uppercase tracking-[0.28em] text-white/55"><?php echo e($panelLabel); ?></p>
                                <h1 class="mt-2 text-xl font-extrabold leading-tight text-white"><?php echo e($schoolBrandName); ?></h1>
                                <p class="mt-1 text-sm text-slate-200/80"><?php echo e($singleSchoolMode && $currentUser->isSuperAdmin() ? 'School Owner' : $currentUser->role->label()); ?></p>
                            </div>
                        </div>
                ...
                <div class="flex-1 overflow-y-auto px-4 py-5">
                    <nav class="space-y-6">
                        <?php $__currentLoopData = $navSections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div>
                                <p class="px-3 text-[11px] font-semibold uppercase tracking-[0.32em] text-white/45"><?php echo e($section['label']); ?></p>
                                <div class="mt-3 space-y-1.5">
                                    <?php $__currentLoopData = $section['items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        ...
                                        <?php if(!empty($item['children'])): ?>
                                            <details class="group rounded-2xl border <?php echo e($isActive ? 'border-[#DFE753]/60 bg-white/6' : 'border-white/8 bg-white/[0.03]'); ?>" <?php echo e($isActive ? 'open' : ''); ?>>
                                                <summary class="flex cursor-pointer list-none items-center gap-3 rounded-2xl px-3.5 py-3 text-sm font-semibold transition duration-200 <?php echo e($isActive ? 'text-[#DFE753]' : 'text-slate-100/92 hover:bg-white/6 hover:text-white'); ?>">
                                                    <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl <?php echo e($isActive ? 'bg-[#DFE753]/12 text-[#DFE753]' : 'bg-white/8 text-[#DFE753] group-hover:bg-white/12'); ?>">
                                                        <?php echo $renderNavIcon($item['icon']); ?>

                                                    </span>
                                                    <span class="flex-1 leading-relaxed"><?php echo e($item['label']); ?></span>
                                                ...
                                                <div class="space-y-1 px-3 pb-3">
                                                    <?php $__currentLoopData = $item['children']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $child): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                        ...
                                                        ?>
                                                        <a href="<?php echo e($child['route']); ?>" class="group flex items-center gap-3 rounded-xl border px-3 py-2.5 text-sm font-medium transition duration-200 <?php echo e($childActive ? 'border-[#DFE753]/70 bg-[#DFE753] text-[#2D1D5C]' : 'border-transparent text-slate-200/88 hover:border-white/8 hover:bg-white/8 hover:text-white'); ?>">
                                                            <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/6 text-[#DFE753]">
                                                                <?php echo $renderNavIcon($child['icon']); ?>

                                                            </span>
                                                            <span class="leading-relaxed"><?php echo e($child['label']); ?></span>
                                                        </a>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                </div>
                                            </details>
                                        <?php else: ?>
                                            <a href="<?php echo e($item['route']); ?>" class="group flex items-center gap-3 rounded-2xl border px-3.5 py-3 text-sm font-medium transition duration-200 <?php echo e($isActive ? 'border-[#DFE753] bg-[#DFE753] text-[#2D1D5C] shadow-lg shadow-black/10' : 'border-transparent text-slate-100/92 hover:border-white/10 hover:bg-white/8 hover:text-white'); ?>">
                                                <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl <?php echo e($isActive ? 'bg-[#2D1D5C]/10 text-[#2D1D5C]' : 'bg-white/8 text-[#DFE753] group-hover:bg-white/12'); ?>">
                                                    <?php echo $renderNavIcon($item['icon']); ?>

                                                </span>
                                                <span class="flex-1 leading-relaxed"><?php echo e($item['label']); ?></span>
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-4 w-4 opacity-60 <?php echo e($isActive ? 'text-[#2D1D5C]' : 'text-white/55 group-hover:text-[#DFE753]'); ?>"><path stroke-linecap="round" stroke-linejoin="round" d="m9 6 6 6-6 6"/></svg>
                                            </a>
                                        <?php endif; ?>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </div>
                ...
                <div class="border-t border-white/10 px-4 py-4 space-y-2">
                    <?php
                        ...
                        $logoutRoute   = ($currentUser->isStudent() || $currentUser->isParent())
                            ...
                            : route('logout');
                    ?>
                    <a href="<?php echo e($profileRoute); ?>" class="flex items-center gap-3 rounded-2xl border px-3.5 py-3 text-sm font-medium transition duration-200 <?php echo e($profileActive ? 'border-[#DFE753] bg-[#DFE753] text-[#2D1D5C]' : 'border-white/10 bg-white/5 text-white hover:border-[#DFE753] hover:bg-[#DFE753] hover:text-[#2D1D5C]'); ?>">
                        <span class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl <?php echo e($profileActive ? 'bg-[#2D1D5C]/10' : 'bg-white/8'); ?>">
                    ...
                    <form action="<?php echo e($logoutRoute); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                    ...
                    <div class="flex items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <h2 class="mt-1 text-xl font-extrabold tracking-tight text-slate-900"><?php echo $__env->yieldContent('header', 'Dashboard'); ?></h2>
                            </div>
                        </div>

                        <div class="flex items-center gap-3 sm:gap-4">
                            <a href="<?php echo e(($currentUser->isStudent() || $currentUser->isParent()) ? route('portal.profile.show') : route('profile.show')); ?>" class="hidden items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 shadow-sm transition hover:border-[#2D1D5C]/30 hover:shadow-md md:flex">
                                <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#2D1D5C] text-sm font-bold text-white"><?php echo e(\Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($currentUser->first_name, 0, 1) . \Illuminate\Support\Str::substr($currentUser->last_name, 0, 1))); ?></span>
                                <div>
                                    <p class="text-sm font-semibold text-slate-900"><?php echo e($currentUser->full_name); ?></p>
                                    <p class="text-xs text-slate-500"><?php echo e($singleSchoolMode && $currentUser->isSuperAdmin() ? 'Administrator' : $currentUser->role->label()); ?></p>
                                </div>
                            </a>
                            <?php
                                $bellCount = 0;
                            ...
                            ?>
                            <a href="<?php echo e($bellRoute); ?>" class="relative inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 bg-white text-[#2D1D5C] shadow-sm transition hover:border-[#2D1D5C]/40 hover:shadow-md">
                                ...
                                <?php if($bellCount > 0): ?>
                                    <span class="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white ring-2 ring-white">
                                        <?php echo e($bellCount > 99 ? '99+' : $bellCount); ?>
        ...
        <div class="<?php echo e($mainContentClass); ?> flex-1">
            ...
            <?php if($errors->any() && !Route::is('login')): ?>
            <div class="mb-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-red-700 shadow-sm">
            ...
            <footer class="mt-auto" style="background-color: #2D1D5C;">
                <div class="mx-auto max-w-7xl px-6 lg:px-8">
                    <div class="border-t pt-5 pb-5" style="border-top-color: rgba(223,231,83,0.35);">
                        <p class="text-xs" style="color: rgba(255,255,255,0.55);">&copy; <?php echo e(date('Y')); ?> <?php echo e($schoolBrandName); ?>. All rights reserved.</p>
                    </div>
                </div>
            </footer>
            </main>
