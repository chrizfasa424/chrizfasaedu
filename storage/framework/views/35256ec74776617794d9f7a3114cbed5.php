<?php $__env->startSection('title', 'Academic Sessions & Terms'); ?>
<?php $__env->startSection('header', 'Academic Sessions & Terms'); ?>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-4 bg-slate-50 border-b border-slate-200">
            <div class="flex items-center gap-3">
                <button onclick="toggleSession('<?php echo e($session->id); ?>')"
                    class="flex items-center gap-2 text-left group">
                    <svg id="chevron-<?php echo e($session->id); ?>" class="h-4 w-4 text-slate-400 transition-transform rotate-180"
                        fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                    </svg>
                    <div>
                        <span class="font-bold text-slate-800 group-hover:text-indigo-700"><?php echo e($session->name); ?></span>
                        <span class="ml-2 text-xs text-slate-400">
                            <?php echo e($session->start_date?->format('d M Y')); ?> – <?php echo e($session->end_date?->format('d M Y')); ?>
            ...
            <div class="flex items-center gap-2">
                <span class="text-xs text-slate-400"><?php echo e($session->terms->count()); ?> term(s)</span>
                <?php if (! ($session->is_current)): ?>
                <form method="POST" action="<?php echo e(route('academic.sessions.set-current', $session)); ?>" class="inline">
                    <?php echo csrf_field(); ?>
                ...
                <?php endif; ?>
                <button onclick="openAddTermModal(<?php echo e($session->id); ?>)"
        ...
        <div id="session-<?php echo e($session->id); ?>">
            <?php if($session->terms->isEmpty()): ?>
                <div class="px-5 py-6 text-center text-sm text-slate-400">
                    No terms yet.
                    <button onclick="openAddTermModal(<?php echo e($session->id); ?>)" class="ml-1 text-indigo-600 hover:underline font-medium">Add a term</button>
                </div>
            <?php else: ?>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <?php $__currentLoopData = $session->terms->sortBy('start_date'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $term): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="hover:bg-slate-50">
                            <td class="px-5 py-3 font-semibold text-slate-800"><?php echo e($term->name); ?></td>
                            <td class="px-5 py-3 text-slate-600"><?php echo e($term->start_date?->format('d M Y') ?? '—'); ?></td>
                            <td class="px-5 py-3 text-slate-600"><?php echo e($term->end_date?->format('d M Y') ?? '—'); ?></td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-3">
                                    <?php if (! ($term->is_current)): ?>
                                    <form method="POST" action="<?php echo e(route('academic.terms.set-current', $term)); ?>" class="inline">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit"
                                    ...
                                    <button onclick="openEditTermModal(
                                        <?php echo e($term->id); ?>,
                                        '<?php echo e(addslashes($term->name)); ?>',
                                        '<?php echo e($term->start_date?->format('Y-m-d')); ?>',
                                        '<?php echo e($term->end_date?->format('Y-m-d')); ?>')"
                                        class="text-xs font-medium text-indigo-600 hover:text-indigo-800 hover:underline">
                                        Edit
                                    </button>
                                    <?php if (! ($term->is_current)): ?>
                                    <form method="POST" action="<?php echo e(route('academic.terms.destroy', $term)); ?>"
                                          onsubmit="return confirm('Delete term \'<?php echo e($term->name); ?>\'? This cannot be undone.')">
...
<div id="create-session-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        ...
        <form method="POST" action="<?php echo e(route('academic.sessions.store')); ?>" class="space-y-4">
            <?php echo csrf_field(); ?>
            <div>
...
function openAddTermModal(sessionId) {
    ...
    form.action = '<?php echo e(url("academic/sessions")); ?>/' + sessionId + '/terms';
    form.reset();
...
function openEditTermModal(termId, name, startDate, endDate) {
    ...
    document.getElementById('edit-term-form').action = '<?php echo e(url("academic/terms")); ?>/' + termId;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\chrizfasaedu\resources\views/academic/sessions/index.blade.php ENDPATH**/ ?>