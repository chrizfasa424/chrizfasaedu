...
<div class="space-y-6">
    ...
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-700"><?php echo e($pendingTestimonials->total()); ?></span>
        </div>

        <form method="POST" action="<?php echo e(route('system.testimonials.bulk')); ?>" id="form-pending">
            <?php echo csrf_field(); ?>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $__empty_1 = true; $__currentLoopData = $pendingTestimonials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-3">
                                    <input type="checkbox" name="ids[]" value="<?php echo e($t->id); ?>" class="row-check rounded border-slate-300" data-group="pending">
                                </td>
                                <td class="px-4 py-3 font-medium text-slate-900"><?php echo e($t->full_name); ?></td>
                                <td class="px-4 py-3 text-slate-500"><?php echo e($t->role_title ?: '—'); ?></td>
                                <td class="px-4 py-3 font-semibold text-amber-600"><?php echo e(max(1,min(5,(int)$t->rating))); ?>/5</td>
                                <td class="px-4 py-3 text-slate-700"><?php echo e(\Illuminate\Support\Str::limit($t->message, 110)); ?></td>
                                <td class="px-4 py-3 text-slate-400 whitespace-nowrap"><?php echo e($t->created_at?->diffForHumans()); ?></td>
                                <td class="px-4 py-3">
                                    <div class="flex flex-wrap gap-2">
                                        <button type="button"
                                            onclick="doAction('<?php echo e(route('system.testimonials.approve', $t->id)); ?>')"
                                            class="rounded-lg bg-emerald-600 hover:bg-emerald-700 px-3 py-1.5 text-xs font-semibold text-white">Approve</button>
                                        <button type="button"
                                            onclick="doAction('<?php echo e(route('system.testimonials.reject', $t->id)); ?>')"
                                            class="rounded-lg bg-slate-500 hover:bg-slate-600 px-3 py-1.5 text-xs font-semibold text-white">Reject</button>
                                        <button type="button"
                                            onclick="doAction('<?php echo e(route('system.testimonials.destroy', $t->id)); ?>',true)"
        ...
        <div class="px-6 py-4 border-t border-slate-100"><?php echo e($pendingTestimonials->links()); ?></div>
    </section>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-700"><?php echo e($approvedTestimonials->total()); ?></span>
        </div>

        <form method="POST" action="<?php echo e(route('system.testimonials.bulk')); ?>" id="form-approved">
            <?php echo csrf_field(); ?>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $__empty_1 = true; $__currentLoopData = $approvedTestimonials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-3">
                                    <input type="checkbox" name="ids[]" value="<?php echo e($t->id); ?>" class="row-check rounded border-slate-300" data-group="approved">
                                </td>
                                <td class="px-4 py-3 font-medium text-slate-900"><?php echo e($t->full_name); ?></td>
                                <td class="px-4 py-3 font-semibold text-amber-600"><?php echo e(max(1,min(5,(int)$t->rating))); ?>/5</td>
                                <td class="px-4 py-3 text-slate-700"><?php echo e(\Illuminate\Support\Str::limit($t->message, 110)); ?></td>
                                <td class="px-4 py-3 text-slate-400 whitespace-nowrap"><?php echo e($t->reviewed_at?->diffForHumans() ?? '—'); ?></td>
                                <td class="px-4 py-3">
                                    <div class="flex flex-wrap gap-2">
                                        <button type="button"
                                            onclick="doAction('<?php echo e(route('system.testimonials.reject', $t->id)); ?>')"
        ...
        <div class="px-6 py-4 border-t border-slate-100"><?php echo e($approvedTestimonials->links()); ?></div>
    </section>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="rounded-full bg-red-100 px-3 py-1 text-xs font-bold text-red-700"><?php echo e($rejectedTestimonials->total()); ?></span>
        </div>

        <form method="POST" action="<?php echo e(route('system.testimonials.bulk')); ?>" id="form-rejected">
            <?php echo csrf_field(); ?>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $__empty_1 = true; $__currentLoopData = $rejectedTestimonials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-3">
                                    <input type="checkbox" name="ids[]" value="<?php echo e($t->id); ?>" class="row-check rounded border-slate-300" data-group="rejected">
                                </td>
                                <td class="px-4 py-3 font-medium text-slate-900"><?php echo e($t->full_name); ?></td>
                                <td class="px-4 py-3 text-slate-700"><?php echo e(\Illuminate\Support\Str::limit($t->message, 110)); ?></td>
                                <td class="px-4 py-3 text-slate-400 whitespace-nowrap"><?php echo e($t->reviewed_at?->diffForHumans() ?? '—'); ?></td>
        ...
        <div class="px-6 py-4 border-t border-slate-100"><?php echo e($rejectedTestimonials->links()); ?></div>
    </section>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\chrizfasaedu\resources\views/system/testimonials/index.blade.php ENDPATH**/ ?>