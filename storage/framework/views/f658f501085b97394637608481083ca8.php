...
<?php $__env->startSection('header', 'Examination Results'); ?>
...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="<?php echo e(route('examination.results.import')); ?>"
            ...
            <?php if(request('class_id') && request('term_id')): ?>
            <a href="<?php echo e(route('examination.results.export', ['class_id' => request('class_id'), 'term_id' => request('term_id')])); ?>"
    ...
    <form method="GET" action="<?php echo e(route('examination.results.index')); ?>"
        ...
        <div>
            ...
            <select name="class_id" required class="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
                ...
                <option value="<?php echo e($c->id); ?>" <?php echo e(request('class_id') == $c->id ? 'selected' : ''); ?>><?php echo e($c->grade_level?->label() ?? $c->name); ?></option>
        ...
        <div>
            ...
            <select name="term_id" required class="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
                ...
                <option value="<?php echo e($t->id); ?>" <?php echo e(request('term_id') == $t->id ? 'selected' : ''); ?>>
        ...
        <?php if(request()->hasAny(['class_id','term_id'])): ?>
        <a href="<?php echo e(route('examination.results.index')); ?>" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">Clear</a>
        <?php endif; ?>
    </form>

    
    <?php if(request('class_id') && request('term_id') && $results->count()): ?>
    <div class="flex flex-wrap gap-2">
        <form method="POST" action="<?php echo e(route('examination.results.compute')); ?>">
            <?php echo csrf_field(); ?>
        ...
        <form method="POST" action="<?php echo e(route('examination.results.approve')); ?>">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="class_id" value="<?php echo e(request('class_id')); ?>">
            <input type="hidden" name="term_id" value="<?php echo e(request('term_id')); ?>">
    ...
    <div class="space-y-3">
        <?php $__currentLoopData = $results; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $studentId => $studentResults): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-3 bg-slate-50 border-b border-slate-200 cursor-pointer"
                 onclick="toggleRow('student-<?php echo e($studentId); ?>')">
                <div class="flex items-center gap-3">
                    <svg id="chevron-<?php echo e($studentId); ?>" class="h-4 w-4 text-slate-400 transition-transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                    <div>
                        <span class="font-semibold text-slate-800"><?php echo e($student?->full_name ?? 'Unknown'); ?></span>
                        <span class="ml-2 text-xs text-slate-400"><?php echo e($student?->admission_number); ?></span>
                    </div>
                </div>
                <div class="flex flex-wrap items-center gap-4 text-xs text-slate-600">
                    <span><span class="font-semibold text-slate-800"><?php echo e($studentResults->count()); ?></span> subjects</span>
                    <span>Total: <span class="font-semibold text-slate-800"><?php echo e(number_format($total, 1)); ?></span></span>
                    <span>Avg: <span class="font-semibold text-slate-800"><?php echo e($avg); ?></span></span>
                    <?php if($reportCard): ?>
                        <span>Pos: <span class="font-semibold text-slate-800"><?php echo e($reportCard->position_in_class); ?>/<?php echo e($reportCard->class_size); ?></span></span>
                    <?php endif; ?>
                    <?php if($approved): ?>
                    ...
                    <?php endif; ?>
                    <?php if($reportCard): ?>
                    <a href="<?php echo e(route('examination.results.report-card', [$studentId, request('term_id')])); ?>"
            ...
            <div id="student-<?php echo e($studentId); ?>" class="hidden overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <?php $__currentLoopData = $studentResults->sortBy('subject.name'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $result): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                        ...
                        ?>
                        <tr class="hover:bg-slate-50" id="result-row-<?php echo e($result->id); ?>">
                            <td class="px-5 py-2.5 font-medium text-slate-800"><?php echo e($result->subject?->name); ?></td>
                            <td class="px-4 py-2.5 text-center text-slate-600"><?php echo e((int)$result->ca1_score); ?></td>
                            <td class="px-4 py-2.5 text-center text-slate-600"><?php echo e((int)$result->ca2_score); ?></td>
                            <td class="px-4 py-2.5 text-center text-slate-600"><?php echo e((int)$result->exam_score); ?></td>
                            <td class="px-4 py-2.5 text-center font-bold text-slate-800"><?php echo e((int)$result->total_score); ?></td>
                            <td class="px-4 py-2.5 text-center text-slate-500 text-xs">
                                ...
                                ?>
                                <?php echo e($pos ? $pos . $suffix : '—'); ?>

                            </td>
                            <td class="px-4 py-2.5 text-center">
                                <span class="rounded-full px-2.5 py-0.5 text-xs font-bold <?php echo e($gc); ?>"><?php echo e($result->grade); ?></span>
                            </td>
                            <td class="px-4 py-2.5 text-center text-xs text-slate-500"><?php echo e($result->teacher_remark); ?></td>
                            <td class="px-4 py-2.5 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick="openEditModal(<?php echo e($result->id); ?>, '<?php echo e(addslashes($result->subject?->name)); ?>', <?php echo e((float)$result->ca1_score); ?>, <?php echo e((float)$result->ca2_score); ?>, <?php echo e((float)$result->exam_score); ?>)"
                                        class="text-xs text-indigo-600 hover:text-indigo-800 font-medium">Edit</button>
                                    <form method="POST" action="<?php echo e(route('examination.results.destroy', $result)); ?>"
    ...
    <?php elseif(request('class_id') && request('term_id')): ?>
    <div class="rounded-2xl border border-dashed border-slate-300 bg-white py-12 text-center text-slate-400">
        ...
        <div class="mt-3">
            <a href="<?php echo e(route('examination.results.import')); ?>" class="text-sm text-indigo-600 hover:underline">Import results from CSV/Excel</a>
        </div>
    </div>
    <?php else: ?>
    <div class="rounded-2xl border border-dashed border-slate-300 bg-white py-12 text-center text-slate-400">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\chrizfasaedu\resources\views/examination/results/index.blade.php ENDPATH**/ ?>